...
//            HrstApplication.getInstance().exit();
        }
    }
     ...
     */
    public static boolean checkNumberAvailable(String number, String textName,
                                               TextView tvTargetNumber, EditText etPhoneNumber) {
        // 当输入的号码长度等于1时
        if (number.equals("")) {
        	tvTargetNumber.setText(textName);
            return false;
        }
        // todo 带国家码, 目前只针对中国 后续有其他业务再兼容, 目前重构先走完主线
        String countryNum = "+86";
        if (InduceConfigure.countryNum.contains("86")) {
        	int length = 11;	//默认长度11位
        	if(number.length() >= 10){
        		//号码长度达到10位时再做物联网号码判断，减少判断次数
        		length = isInternetNumber(number) ? 13 : 11;
            	//动态改变输入框输入长度
        		etPhoneNumber.setFilters(new InputFilter[] { new InputFilter.LengthFilter(length) });
        	}
        	
            if ((!number.substring(0, 1).equals("1"))) {// 判断号码长度为1时的情况
                tvTargetNumber.setText(textName + "(" + "输入首位不正确!" + ")");
                return false;
            } else if (number.length() > 0 && number.length() < length) {
                tvTargetNumber.setText(textName + "(" + "长度不够!" + ")");
            } else if (number.length() == length) {
                tvTargetNumber.setText(textName);
                return true;
            } else if (number.length() > length) {// 如果输入的号码长度大于11时，给予客户提示
                tvTargetNumber.setText(textName + "(" + "输入的号码长度不对" + ")");
                return false;
            } else if (number.length() == 0) {
            	 tvTargetNumber.setText(textName);
    ...
    /**
     * 物联网号码判断
     * 	17400~17405 中国电信集团公司
     * 	17406~17412 工业和信息化部应急通信保障中心
     * 	199(0~9) 中国电信集团公司
     * 	198(0~9) 中国移动通信集团公司
     * 	166(0~9) 中国联合网络通信集团有限公司
     * 	148(0~9) 中国移动通信集团公司
     * 	146(0~9) 中国联合网络通信集团有限公司
     * 	1410(0~9) 中国电信集团公司
     * 	1440(0~9) 中国移动通信集团公司
     * @param number
     * @return
     */
    public static boolean isInternetNumber(String number){
    	if(number == null){
    		return false;
    	}
    	if(number.length() < 5){
    	...
    	if(number.startsWith("199") || number.startsWith("198") || 
    			number.startsWith("166") || number.startsWith("148") ||
    			number.startsWith("146") || number.startsWith("11410") ||
    			number.startsWith("1440") || number.startsWith("1740") ||
    			number.startsWith("17410") || number.startsWith("17411") || number.startsWith("17412")){
    		return true;
    	}
