...
public class SearchRequest implements Parcelable {
    ...
    public static class Builder {
        ...
        public Builder searchBluetoothLeDevice(int durationMs, int times) {
            for (int i = 0; i < times; i++) {
                searchBluetoothLeDevice(durationMs);
            }
            return this;
        }

        public Builder searchBluetoothClassicDevice(int durationMs) {
            SearchTask search = new SearchTask();
            search.setSearchType(SEARCH_TYPE_CLASSIC);
            search.setSearchDuration(durationMs);
            tasks.add(search);
            return this;
        }

        public Builder searchBluetoothClassicDevice(int durationMs, int times) {
            for (int i = 0; i < times; i++) {
                searchBluetoothClassicDevice(durationMs);
            }
            return this;
        }
