...
public class VerifyUtils {
     ...
     * @return
     */
    public static boolean isChineseCard(@NonNull String data) {
     ...
     * @return
     */
    public static boolean isPostCode(@NonNull String data) {
        String expr = "^[0-9]{6,10}";
        return data.matches(expr);
    }

    /**
     * verify ip:port is valid
     */
    public static boolean isIpPortValid(@NonNull String host) {
        String [] ipPort = host.split(":");
        String ipExpr = "^(2(([0-5]{0,2}))\\.|1\\d{0,2}\\.){3}(2(([0-5]{0,2}))\\.|1\\d{0,2})";
        String portExpr = "6([0-5]{0,2})[0-3][0-5]|[1-5]\\d{0,4}|\\d{0,4}$";
        if (ipPort.length == 1){
            return ipPort[0].matches(ipExpr);
        }else if(ipPort.length == 2) {
            return ipPort[0].matches(ipExpr) && ipPort[1].matches(portExpr);
        }else {
            return false;
        }
    }
